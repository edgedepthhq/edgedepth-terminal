...
constexpr int64_t start = CandleBubbleHistory::tile_ms * 100;
constexpr int64_t now = start + CandleBubbleHistory::tile_ms + CandleBubbleHistory::settle_ms;
const Terminal::Pair pair{"binancef","btcusdt"};
Json request(CandleBubbleHistory& h,double clock=1000) {
    auto first = h.request(pair,start,start+60000,now,clock);
    if (!first.is_null()) return first;
    return h.request(pair,start,start+60000,now,clock+400);
}
Json response(const Json& req) {
    Json mins = Json::array();
    for (int64_t ms=start;ms<start+CandleBubbleHistory::tile_ms;ms+=60000)
        mins.push_back({{"start_ms",ms},{"observed",0},{"prints",Json::array()}});
    mins[0]["observed"]=1;
    mins[0]["prints"].push_back({{"timestamp_ms",start+1},{"price",100},{"qty",2},{"is_buy",true},{"id","9007199254740993"}});
    return {{"request_id",req["data"]["request_id"]},{"exchange","binancef"},{"symbol","btcusdt"},
        {"result",{{"from_ms",start},{"to_ms",start+CandleBubbleHistory::tile_ms},{"auto_floor",100},
                   {"per_minute",16},{"minutes",mins}}}};
}
int main() {
    CandleBubbleHistory h;
    auto req=request(h);assert(h.loading());
    assert(h.request(pair,start,start+60000,now,1800).is_null());
    ...
    int loaded,observed;h.coverage(start,start+900000,loaded,observed);assert(loaded==15 && observed==1);
    assert(h.request(pair,start,start+60000,now,5000).is_null()); // cached pan
    std::vector<const Terminal::Trade*> visible;
    h.append_visible(visible,start,start+60000);assert(visible.size()==1);
    ...
    CandleBubbleHistory::receive(result);assert(h.tiles.empty() && !h.error.empty());
    h.reset();req=request(h,14000);h.request(pair,start,start+60000,now,27000);
    ...
    h.request(pair,start,start+60000,now+CandleBubbleHistory::lookback_ms,65000);
    assert(h.tiles.empty() && !h.contains(9007199254740993LL));
    // One request loads four sections, cached independently for later pans.
    h.reset();
    const auto end=start+4*CandleBubbleHistory::tile_ms;
    h.request(pair,start,end,end+CandleBubbleHistory::settle_ms,70000);
    req=h.request(pair,start,end,end+CandleBubbleHistory::settle_ms,70400);
    assert(req["data"]["from_ms"]==start && req["data"]["to_ms"]==end);
    result=response(req);result["result"]["to_ms"]=end;
    for(int64_t ms=start+CandleBubbleHistory::tile_ms;ms<end;ms+=60000)
    ...
    assert(h.request(pair,start,end,end+CandleBubbleHistory::settle_ms,71000).is_null());
    h.coverage(start,end,loaded,observed);assert(loaded==60 && observed==1);
