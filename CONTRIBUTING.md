...
- `snake_case` for functions/variables, C++20, no exceptions in hot paths.
...
- All colors come from `Theme::Tokens::*` — no hardcoded `ImVec4` colors in widgets. New colors start life in [`design/tokens.json`](design/README.md), then get mirrored into the theme.
- Match the surrounding style; this codebase favors explicitness over cleverness.

## What we're most interested in

- Feed adapters and gateway implementations for other exchanges/venues
- Indicators and widget improvements
- Build/tooling portability fixes
...
This project is maintained alongside a running product. Triage happens in batches, typically weekly — a quiet few days doesn't mean your PR is ignored. Issues are welcome, but this is not a support contract; "doesn't build" reports need the info in the issue template or they'll be closed.
